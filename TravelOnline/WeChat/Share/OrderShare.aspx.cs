...
{
    public partial class OrderShare : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Convert.ToString(Session["Online_UserId"]).Length > 0)
            {
                ...
                if (DS.Tables[0].Rows.Count > 0)
                {
                    ...
                    planid = ConfigurationManager.AppSettings["2017-12_" + LineId + "_planid"];
                    begindate = ConfigurationManager.AppSettings["2017-12_" + LineId + "_begindate"];
                    try
                    {
                    ...
                    string DefaultOption = string.Format("value=0 max=4 min=0");
                    string DefaultChildOption = string.Format("value=0 max=4 min=0");
                    string AdultOption = string.Format("value=1 max=4 min=0");
                    string ChildOption = string.Format("value=0 max=4 min=0");
                    ...
                    if (GetPlan.PlanStaPrice != null)
                    {
                        ...
                        for (int i = 0; i < GetPlan.PlanStaPrice.Length; i++)
                        {
                            ...
                            if (!lstPriceType.Contains("儿童价"))
                            {
                                ...
                                {
                                    AdultOption = string.Format("value=1 max=4 min=0");
                        ...
                        if (NoNeedPrice > 0)
                        {
                            ...
                            for (int i = 0; i < GetPlan.PlanExtPrice.Length; i++)
                            {
                                ...
                                if (GetPlan.PlanExtPrice[i].OnlineNeeds == "0")
                                {
                                    ...
                                    Strings.Append("<div class=\"col-xs-5\">");
                                    Strings.Append("<div style=\"width:100px\">");
                                    Strings.Append(string.Format("<input tps=ExtPrice tagid=\"{0}\" price=\"{1}\" type=\"text\" class=\"form-control\" readonly value=1 max=4 min=0 /></div>", GetPlan.PlanExtPrice[i].PriceId, GetPlan.PlanExtPrice[i].Price));
                                    Strings.Append("");
                                    Strings.Append("</div>");

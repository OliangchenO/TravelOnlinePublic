...
function ShowOrderDetail_New(data) {
    ...
    if (data.rows != null) {
        ...
        if (data.row1 != null) {
            ...
            if (data.rows[0].PayType == "1" && data.rows[0].OrderFlag == "1" && data.rows[0].PayFlag == "0" && Number(data.buytime) < 1) {
                html += "<A class='btn yellow' href='/wechat/pay_tsw.aspx?OrderId=" + data.orderid + "' target='_blank'>在线支付</A>";
            }
...
$('#goPay').live("click", function () {
    ...
    var url = "../../WeChat/AjaxService.aspx?action=getEncryptDes&phoneNumber=" + mobile;
    $.get(url, function (data) {
        var o = eval("(" + data + ")");
        if (o.success) {
            url = "https://shyouthact.net/web/user/scytsMobileValidate";
            var dataJson = o.encryStr
            $.ajax({
                type: "POST",
                dataType: "json",
                async: false,
...
function orderForTsw() {
    url = "../../WeChat/AjaxService.aspx?action=orderTotsw";
    $.post(url, $("#form_data").serialize(), function (obj) {
        if (obj.success) {
            if (obj.success == "OK") {
                window.location.href = "/wechat/pay_tsw.aspx?OrderId=" + obj.orderid;
            }
        }
        else {

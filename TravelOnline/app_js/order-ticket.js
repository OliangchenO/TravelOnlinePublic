...
$(document).ready(function () {
    if (state.action == "login") {
        $.cookie("loginstep", "orderTicket", { expires: 30, path: '/WeChat' });
        $.cookie("lineid", lineid, { expires: 30, path: '/WeChat' });
...
function orderTicket() {
    if (typeof ($("#preferCode").val()) == "undefined") {
        preferCode = "";
    } else {
...
$('#weixinLogin').live("click", function () {
    top.location = "/WeChat/WeiXinLogin.aspx?state=Wx_regedit_ticket&LineId=" + $.cookie("lineid");
})
